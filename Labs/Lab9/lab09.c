...
uint32_t push_lsb_bit(uint32_t bitbucket, uint32_t bit) {
  // shift left then compare with added bit
 ...
 */
uint32_t isolate_bits(uint32_t bitbucket, uint32_t msbindex,
                      uint32_t lsbindex) {
  uint32_t mask = (1 << (msbindex - lsbindex + 1)) - 1; // 0001 1111
  bitbucket = bitbucket >> lsbindex;                    // 0011 1101
  return bitbucket & mask;                              // 0001 1101
}
...
int main() {
  ...
  uint32_t temp = 13;
  uint32_t temp1 = 20;
  printf("Testing pop_lsb_bit( 13 ): %d\n", pop_lsb_bit(&temp));
  print_b32(temp);

  printf("Testing pop_lsb_bit( 13 ): %d\n", pop_lsb_bit(&temp1));
  print_b32(temp1);
  uint32_t y = push_lsb_bit(20, 1);
...
#if 1
  /*********** CHECKPOINT 3 ***********/
  uint32_t inst[] = {45459490,  66803754, 273938261, 12965920,
                     137929722, 45459492, 45459493};
